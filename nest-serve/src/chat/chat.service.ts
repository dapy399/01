...
export class ChatService {
  ...
  async combineConvo(
    ...
    isWebSearch?: boolean | undefined, //是否开启联网搜索
  ) {
    ...
    if (isWebSearch) {
      ...
      this.notifStream(stream, {
        ...
        promptInfo: '正在联网搜索最新资讯',
        fileList: [],
      });

      // 调用搜索服务
      webSearchResult = await this.searchService.searchTavily(content);
    ...
    // ========== 整合联网搜索结果到用户消息 ==========
    if (webSearchResult) {
      messages.content = `${webSearchResult}\n\n用户问题：${content}`;
      messages.displayContent = content;
    }
    // ================================================
  ...
  async callingModel(
    ...
    let systemPrompt = medAssistantDataPrompt;

    // 如果开启了联网搜索，追加提示词
    if (isWebSearch) {
      systemPrompt +=
        '\n\n【联网搜索模式】你已获取最新网络资讯，请结合搜索结果和医学知识综合回答。注意区分已有知识和最新信息，对于最新信息请标注"根据最新搜索结果"。';
    }
  ...
  async queryKb(
    ...
    let assistantMessage = '';
    // 再次调用模型回复用户
    const res2 = await this.callingModel(messageList, false, false, controller);
